  ...
  <div class="main31">
    ...
    <div class="panel panel-primary" style="background-color: lightyellow">
      ...
      <div class="mainBody31">
        ...
        >
          <td style="height: 50px ; font-size:20px">用户学号</td>
      ...
      <div class="mainBottom">
        <h1>
          ...
          <input type="submit" value="提交" class="btn btn-default" @click="userBorrowCount" />
...
export default {
  ...
  methods: {
    ...
    userBorrowCount() {
      if (this.size < 16 && this.current > 0) {
        if (this.userGraduationYear > 0 || this.userGraduationYear == "") {
          this.$http
            .post("http://localhost:8081/Statistics/userBorrowCount", {
              size: this.size,
    ...
    exportTable() {
      this.$http({
        url: "http://localhost:8081/Statistics/userBorrowCountExportDownload",
        method: "post",
        data: {
          size: this.size,
          current: this.current,
        ...
        .then(response => {
          ...
          let filename = "各专业借书统计表第" + this.current + "页.xls";
